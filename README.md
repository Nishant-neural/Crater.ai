...
Crater now has an explicit revision-wide integration stage. After chunk-level
extraction persists universal facts, `POST /knowledge/revisions/{revision_id}/integrate`
loads the complete persisted knowledge set (universal facts, legacy procedures/failure
modes, schematic evidence and expert knowledge), asks the LLM to resolve aliases and
